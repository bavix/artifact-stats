        ...
        for row in reader:
            ...
            points_by_repo[repo].append(
                Point(
                    ...
                    forks=int(float(row.get("forks", "0") or 0)),
                )
            )

    if not points_by_repo:
        raise ValueError("No points found in CSV")
    for repo, items in points_by_repo.items():
        items.sort(key=lambda point: point.ts)
        if not items:
            raise ValueError(f"No points found for {repo}")
    return points_by_repo


def build_metric_svg(
    points_by_repo: Dict[str, List[Point]],
    repos: List[str],
    color_map: Dict[str, Tuple[str, str, str]],
    metric_name: str,
    ...
    max_value = max(getattr(point, metric_name) for point in all_points)
    ...
    lines.append(f'<text x="{plot_left}" y="70" fill="#9ca3af" font-size="16" font-family="{FONT_FAMILY}">GitHub {metric_name} by snapshot date</text>')
    ...
    for idx, repo in enumerate(repos):
        ...
        polyline = " ".join(f"{x_coord(point.ts):.2f},{y_coord(getattr(point, metric_name)):.2f}" for point in points)
        lines.append(f'<polyline fill="none" stroke="url(#repo-{idx})" stroke-width="4" stroke-linecap="round" stroke-linejoin="round" points="{polyline}"/>')

        marker_step = max(1, len(points) // 24)
        for point_index, point in enumerate(points):
            if point_index % marker_step != 0 and point_index != len(points) - 1:
                continue
            value = getattr(point, metric_name)
            lines.append(f'<circle cx="{x_coord(point.ts):.2f}" cy="{y_coord(value):.2f}" r="3.8" fill="{point_color}" stroke="#111827" stroke-width="1"/>')
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate GitHub stars/forks charts from CSV")
    parser.add_argument("--input", default="stats/github.csv", help="Input CSV path")
    parser.add_argument("--output-dir", default="svg", help="Output directory for SVG files")
    parser.add_argument("--output-prefix", default="github", help="Output filename prefix")
    parser.add_argument("--title", required=True, help="Base chart title")
    parser.add_argument("--repos", default="", help="Optional comma-separated repositories to draw")
    parser.add_argument("--color", action="append", default=[], help="Repo color map entry: repo=#RRGGBB or repo=#RRGGBB,#RRGGBB")
    parser.add_argument("--colors", default="", help="Optional semicolon-separated color specs")
    parser.add_argument("--source-label", default="stats/github.csv", help="Source label text")
    args = parser.parse_args()

    all_points_by_repo = load_points(Path(args.input))
    ...
    for metric_name, file_suffix in METRICS:
        svg = build_metric_svg(
            points_by_repo=points_by_repo,
            repos=repos,
            color_map=color_map,
            metric_name=metric_name,
            title=f"{args.title} - {metric_name.capitalize()}",
            source_label=args.source_label,
        )
        output_path = output_dir / f"{args.output_prefix}-{file_suffix}.svg"
